...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, x_lr, event):
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            xt_next = at_next.sqrt() * x0_t + c1 * torch.randn_like(event) + c2 * et
            xs.append(xt_next.to('cpu'))
        ret_img = xs
        return ret_img[-1]


    def p_sample_loop_complete(self, x_lr, event):
        device = self.betas.device
        n = x_lr.size(0)
        noise = torch.randn_like(event)
        skip = self.num_timesteps // 5
    ...
    def p_sample_loop_train(self, x_lr, event):
        device = self.betas.device
        n = event.size(0)
        noise = torch.randn_like(event)
        skip = self.num_timesteps // 5
    ...
    def p_losses(self, x_in, noise=None):
        ...
        # difference = x_in['BL'] - x_in['SR'] ######差值图像
        # x_recon = self.denoise_fn(torch.cat([x_in['BL'], difference, x_noisy], dim=1), t.float()) #由Blur和差值图像估计事件
        x_recon = self.denoise_fn(torch.cat([x_in['BL'], x_noisy], dim=1), t.float()) #由Blur事件
        # x_recon = self.denoise_fn(torch.cat([x_in['BL'], x_in['SR'], x_noisy], dim=1), t.float()) #由Blur估计事件
