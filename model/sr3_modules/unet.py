...
# from model.sr3_modules.refineModel import RDSRNet_h
...
class UNet(nn.Module):
    ...
    def forward(self, x, time):
        ...
        # x = torch.cat((x_image, x_noisy), dim=1)

        # x_event = x[:, :6, :, :]
        # x_image = x[:, 6:9, :, :]
        # x_noisy = x[:, 9:, :, :]

        # f_image,f_image_LR = self.refine_net(x_image,x_event)
        # x = torch.cat((f_image, x_noisy), dim=1)
